...
export function SessionCard({ session }: { session: Session }) {
	return (
		<div className="group relative cursor-pointer overflow-hidden rounded-2xl bg-cream-50 shadow-card transition-all duration-300 hover:shadow-hover dark:bg-[#13151A]">
			<div className="relative h-48 overflow-hidden">
				...
				<div className="absolute top-3 right-3 rounded-full bg-cream-50/90 px-3 py-1 text-xs font-medium text-earth-700 dark:bg-[#111318]/90 dark:text-[#E2DBCF]">
				...
				<div className="absolute inset-0 flex items-center justify-center opacity-0 transition-opacity duration-300 group-hover:opacity-100">
					<button className="flex h-16 w-16 items-center justify-center rounded-full bg-white/95 shadow-xl transition-transform hover:scale-110">
						<svg className="ml-1 h-8 w-8 text-sage-600" fill="currentColor" viewBox="0 0 20 20">
			...
			<div className="p-4">
				<h3 className="mb-2 line-clamp-2 font-semibold text-earth-900 text-lg dark:text-[#F4EFE6]">{session.title}</h3>
				<p className="mb-3 line-clamp-2 text-sm text-earth-600 dark:text-[#CFC7BB]">{session.description}</p>
				<div className="flex flex-wrap gap-2">
					{session.mood?.slice(0, 2).map((mood) => (
						<span key={mood} className="rounded-full bg-sage-50 px-3 py-1 text-xs text-sage-700 dark:bg-white/10 dark:text-[#D9D3C8]">
							{mood}
						</span>
					))}
				</div>
...
export function StatCard({ icon, label, value, trend }: StatCardProps) {
	return (
		<div className="rounded-2xl bg-cream-50 p-6 shadow-card transition-shadow hover:shadow-hover dark:bg-[#13151A]">
			...
			<div className="mb-1 text-3xl font-bold text-earth-900 dark:text-[#F4EFE6]">{value}</div>
			<div className="text-sm text-earth-600 dark:text-[#CFC7BB]">{label}</div>
		</div>
	);
}
...
export function ProgramCard({ program }: { program: Program }) {
	return (
		<div className="overflow-hidden rounded-3xl bg-cream-50 shadow-card transition-all duration-300 hover:shadow-hover dark:bg-[#13151A]">
